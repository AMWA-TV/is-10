...
*   Verification of the signature of a software statement as defined in Section 2.3 of [RFC 7591][RFC-7591]
*   Presentation of an initial JSON Web Token (JWT) as suggested in Section 3 of [RFC 7591][RFC-7591]
*   Operator approval after registration via the Authorization Server user interface or API

Where dynamic client registrations request the `client_credentials` grant type they MUST be subject to one of the
explicit authorization steps listed above. Authorization Servers MAY be configured to grant implicit authorization where
only the `authorization_code` grant type is requested.
