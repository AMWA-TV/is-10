...
Authorization Servers MUST provide support for client authentication at the token and revocation endpoints using HTTP
Basic Authentication as per Section 2 of [RFC 2617][RFC-2617], and in the manner described in Section 2.3 of
[RFC 6749][RFC-6749]. This mechanism is identified by the client registration `token_endpoint_auth_method` of
`client_secret_basic`.

Authorization Servers MUST provide support for client authentication at the token and revocation endpoints using a
digitally signed JSON Web Token in the manner described in Section 2.2 of [RFC 7523][RFC-7523]. This mechanism is
