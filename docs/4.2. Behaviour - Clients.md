...
registration. Clients SHOULD be capable of hosting their own public JWKs at a location that corresponds to the value of
the registered `jwks_uri` used when registering with the Authorization Server. Clients are RECOMMENDED to use the
`jwks_uri` in preference to `jwks` such that their public keys can be replaced without re-registering with the
