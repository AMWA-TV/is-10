...
Authorization Framework [RFC 6749][RFC-6749]. All tokens produced by the Authorization Server MUST be Bearer Tokens
using the JSON Web Token (JWT) profile as per [RFC 7523][RFC-7523].
...
*   **/.well-known/oauth-authorization-server** - This path MUST return a response matching the requirements of OAuth
2.0 Authorization Server Metadata [RFC 8414][RFC-8414]. This MUST include the optional `jwks_uri`,
`registration_endpoint` and `revocation_endpoint` parameters.
...
The Authorization Server and Resource Servers MUST correctly reply to OPTIONS requests, explicitly permitting the use of
the `Authorization` header inside the `Access-Control-Allow-Headers` field.
...
Resource Servers SHOULD return error codes and responses in line with Seciton 3 of [RFC 6750][RFC-6750]. OAuth 2.0
