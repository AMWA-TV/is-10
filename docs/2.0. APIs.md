...
The following endpoints MUST be implemented by an Authorization Server at the paths signalled in the Authorization
Server Metadata.
...
*   The `revocation_endpoint`, for example at **/revoke** - Used as the revocation endpoint for revoking currently
    active access tokens or refresh tokens, as defined by [RFC 7009][RFC-7009].
