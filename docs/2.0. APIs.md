...
The Authorization API specified above is one valid implementation, but does not define how every Authorization Server will present itself. Any OAuth 2.0 compatible Authorization Server which implements the following RFCs is compatible with this authorization mechanism:
...
OAuth 2.0 Authorization Framework [RFC-6749]. All tokens produced by the Authorization
Server MUST be Bearer Tokens using the JSON Web Token (JWT) profile as per [RFC-7523].
...
with a value of "no-cache", in line with [RFC-6749].
...
correctly handle a 302 response (Found) as stated in Section 4.1.2 of [RFC-6749] following the
guidelines given in [RFC-7231].
...
The Authorization Server and Resource Servers MUST correctly reply to OPTIONS requests, explicitly permitting the use of the
`Authorization` Header inside the `Access-Control-Allow-Headers` field.
...
Authorization Servers SHOULD return error codes and responses in line with Section 5.2. of [RFC-6749]. OAuth 2.0 Clients
MUST be capable of handling such responses.

Resource Servers SHOULD return error codes and responses in line with Seciton 3 of [RFC-6750]. OAuth 2.0 Clients MUST
be capable of handling such responses.

Where the RAML specification of an API specifies explicit response codes it is expected that a client will handle these cases
in a particular way. As explicit handling of every possible HTTP response code is not expected, clients must instead
implement more generic handling for ranges of response codes (1xx, 2xx, 3xx, 4xx and 5xx). For HTTP codes 400 and upwards, a
JSON format response MUST be returned as follows:
...
string format. The format of the `error` field SHOULD follow one of the error types given by Section 5.2. of [RFC-6749]. `error_description` is an optional human-friendly message describing
