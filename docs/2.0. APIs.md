...
OAuth 2.0 Clients MUST support pre-flight OPTIONS requests when performing all HTTP requests, including GET requests, as the presence of the 
`Authorization` HTTP header means the request is regarded as "non-simple", in line with the W3C's [Cross-Origin Resource Sharing][cross-origin] 
specification.

The Authorization Server and Resource Servers MUST correctly reply to OPTIONS requests, permitting the use of the `Authorization` Header inside 
the `Access-Control-Allow-Headers` field.
