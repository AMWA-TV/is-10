...
The `iss` (issuer) claim MUST be included in the token. The claim MUST contain a case-sensitive string containing a
StringOrURI value that identifies the Authorization Server that issued the JWT. The contents of this claim
SHOULD match one entry in the common name field or alternate names fields in all TLS certificates used by the
Authorization Server for securing the authorization API, in order to allow the verifier of the token to fetch the
public key of the Authorization Server.
...
A '`*`' wildcard can be used to replace zero or more characters (this is equivalent to the `.*` regex pattern), and may 
encompass multiple path segments (e.g. `single*` or `single/senders/*/constraints` would both correspond to
...
Resource Servers SHALL apply [URL Normalization][URL Normalization] in order to prevent double dot URL `..` introducing 
vulnerabilities. The path `/x-nmos/connection/v1.1/single/../bulk` MUST not be permitted by `single/*` path permission.
...
_Note:_ The `*` may or may not be proceded by a slash.
