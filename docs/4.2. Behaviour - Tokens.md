...
[RFC 6749][RFC-6749] Section 4.1.2.

### Access Token Response

Successful access token requests shall be serviced by the Authorization Server as defined in
...
For the authorization grant, the subject typically identifies the authorized accessor for which the
...
contents of which are key:value pairs, with the keys identifying an individual AMWA NMOS API and the value
indicating access permissions for that API. An empty object MUST indicate no access to the specific API. A missing
API key MUST indicate no access to the specific API.
...
In addition, a set of universal key:value pairs can be included, with the keys indicating the type of access to the
API and the value indicating individual paths that are permitted to be accessed in such a way. Wild-carding is
permitted in the path specifiers. Individual AMWA specifications SHOULD also specify additional key:value pairs
...
[RFC-6750]: https://tools.ietf.org/html/rfc6750 "The OAuth 2.0 Authorization Framework: Bearer Token
Usage"
